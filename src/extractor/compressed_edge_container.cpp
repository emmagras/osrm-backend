...
void CompressedEdgeContainer::SerializeInternalVector(const std::string &path) const
{
    ...
    for (auto &geom_index : m_compressed_geometry_index) {
        geometry_out_stream.write((char *)&(geom_index), sizeof(unsigned));
    }
    ...
    for (auto &node_id : m_compressed_geometry_nodes)
    {
        geometry_out_stream.write((char *)&(node_id), sizeof(NodeID));
    }

    unsigned fwd_weight_control_sum = 0;
    // write compressed geometry forward weights's
    for (auto &forward_weight : m_compressed_geometry_fwd_weights)
    {
        geometry_out_stream.write((char *)&(forward_weight), sizeof(EdgeWeight));
    }

    unsigned rev_weight_control_sum = 0;
    // write compressed geometry reverse weights's
    for (auto &reverse_weight : m_compressed_geometry_rev_weights)
    {
        geometry_out_stream.write((char *)&(reverse_weight), sizeof(EdgeWeight));
    }
}
...
unsigned CompressedEdgeContainer::ZipEdges(const EdgeID f_edge_id, const EdgeID r_edge_id)
{
    ...
    const unsigned zipped_geometry_id = m_compressed_geometry_nodes.size();
